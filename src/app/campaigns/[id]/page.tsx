...
export default function CampaignDetail({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchCampaign = async () => {
      try {
        ...
        const contributionsResponse = await fetch(`/api/campaigns/${params.id}/contributions`);
        ...
        const leaderboardResponse = await fetch(`/api/campaigns/${params.id}/contributions?leaderboard=true`);
  ...
  const handleOpenDonationModal = () => {
    if (status === 'unauthenticated') {
      router.push(`/login?callbackUrl=/campaigns/${params.id}`);
      return;
    }
              ...
              {campaign.additionalMedia && campaign.additionalMedia.length > 0 && (
                <Box sx={{ mt: 4 }}>
                  ...
                  <Grid container spacing={2}>
                    {campaign.additionalMedia.map((media: any, index: number) => (
              ...
              {campaign.socials && campaign.socials.length > 0 && (
                <>
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                    {campaign.socials.map((social: any, index: number) => (
                      <Chip 
                        key={index}
